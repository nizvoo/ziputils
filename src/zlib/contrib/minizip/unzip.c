    ...
    uLong commentBufferSize;
{
    ...
    if ((err==UNZ_OK) && (szFileName!=NULL))
    {
        ...
        if ((file_info.size_filename>0) && (fileNameBufferSize>0)) {
            ...
            memset(ascii, 0, sizeof(char) * MAX_PATH);

            if (ZREAD(s->z_filefunc, s->filestream,ascii,uSizeRead)!=uSizeRead)
                err=UNZ_ERRNO;
#ifdef UNICODE
            mbstowcs(szFileName, ascii, strlen(ascii) + 1);
            /*
            index = 0;
            ptr = (char*)szFileName;
            for (i = 0; i < strlen(ascii) / 2; i += 2) {
